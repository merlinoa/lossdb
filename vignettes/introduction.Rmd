...
* id - a unique (by dev) identifier for the claim or occurrence or member
...
* Take a data set (`losses`) in a format resembling how data may be provided by an insurance company, and transforming that data set into a `loss_df` object that contains and organizes all the information relevant to the analysis.  The `losses` data set is provided by the `lossdb` package.
...
The `loss_df()` function takes the data frame columns relevant to the actuarial analysis as its formal arguments.  Before we can create a `loss_df` object we need to transform the `losses` data set into a usable format.
...
Each detail (dollar or desc) column has an attribute (`detail`) specifying the type of loss detail that the column contains.  The `detail` attribute is defined by the argument the column is supplied to in the `loss_df()` function (i.e. `paid_loss` and `paid_expense` are of `detail` `paid`).  These columns maintain the column names that they are supplied with, and are categorized by their `detail`.  The names for `meta` columns are changed to the `meta` category they are supplied.
...
Before a projection is made we must specifiy the loss amounts we wish to project (e.g. paid loss & ALAE gross of all recoveries, paid loss & ALAE net of all recoveries, medical only paid loss & ALAE gross of all recoveries, etc.).  Use the `paid()`, `incurred()`, `paid_recovery()`, and `incurred_recovery()` functions to get the total from each "dollar" category.
...
Examples of actuarial reports created using R are available [here](). 