...
Before a projection is made we must specifiy the loss amounts we wish to project (e.g. paid loss & ALAE gross of all recoveries, net of all recoveries, medical only paid loss & ALAE net of all recoveries, etc.).  The `retain` function specifies the values to be projected.  Like the `loss_df` function, it is an S3 class constructor.
```{r}
