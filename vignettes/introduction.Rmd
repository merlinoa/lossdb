...
The `projection()` function can be used to make simple chain ladder projections.
```{r}
# make a paid projections based of total paid amounts gross of all recoveries
projection(mydf, value = "paid")
```
```{r}
# make a paid projection net of all recoveries
projection(mydf, value = "paid", recovery = "paid_recovery")
```
```{r}
# make a paid projection net of paid_excess250 recoveries
projection(mydf, value = "paid", recovery = "paid_excess250")
```
```{r}
# include recovery detail by category: `sal_sub` and `paid_excess250`
projection(mydf, value = "paid", recovery = c("paid_excess250", "sal_sub"))
```