...
str(losses)
```
There are many columns that are not relevant to our reserving analysis, and we are going to have to apply some transformations to the data to get all the information we need out of the losses data frame.  Let's get started.
...
No let's see a bar chart of the data
```{r}
plot(mydf)
```
...
`mychanges2` is a data frame consisting of all the claims in which there was a change in the "paid_loss_only" column from 2012-06-30 to 2013-06-30.  You can now browse through the changed claim to spot obvious problems with the new data.  For example we may want to check that there are no missing claims (i.e. no claims that were in the data at the last `evaluation_date` that are no longer in the data)

```{r}
# check for missing claims
mychanges2[, mychanges2$claim_cts_change < 0]
```
...
mychanges2[, mychanges2$paid_loss_only_changes < 0]
```
...
Before a projection is made we must specifiy exactly what loss amounts we wish to project (e.g. paid loss & ALAE gross of all recoveries, net of all recoveries, medical only paid loss & ALAE net of all recoveries, etc.).  The `retain` function specifies the values to be projected.  Like the `loss_df` function, it is an S3 class constructor.
```{r}
# create data frame with total paid values gross of all recoveries
retain(mydf, value = "paid")
```
```{r}
# create data frame with total paid values net of total paid recovery
retain(mydf, value = "paid", recovery = "paid_recovery")
```
```{r}
# create a data frame with paid net of paid_excess250 recoveries
retain(mydf, value = "paid", recovery = "paid_excess250")
```
```{r}
# include recovery detail by category: `sal_sub` and `paid_excess250`
retain(mydf, value = "paid", recovery = c("paid_excess250", "sal_sub"))
```